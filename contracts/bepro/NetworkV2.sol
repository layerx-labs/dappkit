...
contract NetworkV2 is Governed {
    ...
    function _isBountyOwner(uint256 id) internal view {
        require(bounties[id].creator == msg.sender, "O1");
    }
    ...
    function _isFundingRequest(uint256 id, bool shouldBe) internal view {
        require((bounties[id].fundingAmount > 0) == shouldBe, "B1");
    }
    ...
    function _isInDraft(uint256 id, bool shouldBe) internal view {
        _bountyExists(id);
        require((block.timestamp < bounties[id].creationDate.add(draftTime)) == shouldBe, "B1");
    }
    ...
    function _proposalExists(uint256 _bountyId, uint256 _proposalId) internal view {
        require(_proposalId <= bounties[_bountyId].proposals.length - 1, "D0");
    }
    ...
    function changeNetworkParameter(uint256 _parameter, uint256 _value) external onlyGovernor {
        if (_parameter == uint256(INetworkV2.Params.councilAmount)) {
            ...
            require(totalNetworkToken == 0, "E1");
    ...
    function delegateOracles(uint256 amount, address toAddress) external {
        _amountGT0(amount);
        require(amount <= oracles[msg.sender].locked, "M0");
        oracles[msg.sender].locked = oracles[msg.sender].locked.sub(amount);
     ...
     */
    function hardCancel(uint256 id)  onlyGovernor external {
        require(bounties[id].creator != address(0), "H1");
        require(block.timestamp.sub(bounties[id].creationDate) >= cancelableTime, "H3");
        ...
        if (bounties[id].proposals.length > 0) {
            for (uint256 i = 0; i <= bounties[id].proposals.length - 1; i++) {
                ...
                require((proposal.disputeWeight >= _toPercent(oraclesDistributed, percentageNeededForDispute)) || proposal.refusedByBountyOwner == true, "H4");
            }
        }
    ...
    function cancelFundRequest(uint256 id)  external {
        ...
        require(bounties[id].funded == false, "C1");
        _isNotCanceled(id);
        _cancelFundingRequest(id);
    }
     ...
     */
    function retractFunds(uint256 id, uint256 fundingId)  external {
        _isInDraft(id, true);
        _isFundingRequest(id, true);
        _isNotCanceled(id);
    ...
    function cancelPullRequest(uint256 ofBounty, uint256 prId) external {
        ...
        require(prId <= bounties[ofBounty].pullRequests.length - 1, "C1");
        require(bounties[ofBounty].pullRequests[prId].canceled == false, "C2");
        require(bounties[ofBounty].pullRequests[prId].creator == msg.sender, "C3");
    ...
    function markPullRequestReadyForReview(uint256 bountyId, uint256 pullRequestId) external {
        ...
        require(pullRequestId <= bounties[bountyId].pullRequests.length - 1, "P1");
        require(bounties[bountyId].pullRequests[pullRequestId].ready == false, "P2");
        require(bounties[bountyId].pullRequests[pullRequestId].creator == msg.sender, "P3");
        require(bounties[bountyId].pullRequests[pullRequestId].canceled == false, "P4");
    ...
    function createBountyProposal(
        ...
        require(oracles[msg.sender].locked.add(oracles[msg.sender].byOthers) >= councilAmount, "C5");
        require(prId <= bounties[id].pullRequests.length - 1, "C0");
        require(bounties[id].pullRequests[prId].ready == true, "C1");
        ...
        require(_total == bounty.tokenAmount, "C1");
    ...
    function disputeBountyProposal(uint256 bountyId, uint256 proposalId) external {
        ...
        require(disputes[msg.sender][b32] == 0, "D1");
    ...
    function closeBounty(uint256 id, uint256 proposalId, string memory ipfsUri)  external {
        ...
        require(block.timestamp >= bounty.proposals[proposalId].creationDate.add(disputableTime), "C2");
        require(proposal.disputeWeight < _toPercent(oraclesDistributed, percentageNeededForDispute), "C3");
        require(proposal.refusedByBountyOwner == false, "C7");
        require(bounties[id].pullRequests[proposal.prId].canceled == false, "C8");
        ...
        for (uint256 i = 0; i <= proposal.details.length - 1; i++) {
            ...
            require(erc20.transfer(detail.recipient, proposalAmount.div(100).mul(detail.percentage)), "C5");
        ...
        require(erc20.transfer(msg.sender, mergerFee), "C4");
        require(erc20.transfer(proposal.creator, proposerFee), "C4");
        if (treasuryAmount > 0) {
